...
class LoggerSingleton:
    ...
    def log_level(self, level: int) -> None:
        ...
        print(
            f"{Fore.BLUE}Уровень логирования изменен на: {logging.getLevelName(level)}{Style.RESET_ALL}"
        )
    ...
    def enable_file_logging(self, file_path: str) -> None:
        ...
        if self._file_logging_enabled and self._log_file_path == file_path:
            print(
                f"{Fore.YELLOW}Запись логов в файл '{file_path}' уже включена{Style.RESET_ALL}"
            )
            return
    ...
    def __str__(self) -> str:
        ...
        file_info = f", запись в файл {status}" + (
            f" ('{self._log_file_path}')" if self._file_logging_enabled else ""
        )
